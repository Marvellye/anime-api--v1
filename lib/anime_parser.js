...
const ajax_url2 = 'https://anitaku.so/';
...
const recent_release_url = `${ajax_url2}home.html`;
...
export const scrapeSchedule = async (baseUrl) => {
  const url = `${baseUrl}/schedule?type=anime&fields=[id,title,coverImage]`;
  try {
...
export const scrapeWatchAnime = async ({ id }) => {
  try {
    ...
    // Scrape the contents from ep_download URL
    const downloadPage = await axios.get(download);
    const download$ = cheerio.load(downloadPage.data);

    // Extract download links
    const downloadLinks = download$('#content-download .dowload a').map((_, element) => ({
    text: download$(element).text(),
    url: download$(element).attr('href')
    })).get();

    // Add download links to the stream array
   // const stream = downloadLinks;
    const pageTitle = download$('title').text();
    ...
    stream = [{ text: "Could not crawl", url: pageTitle }];
    }

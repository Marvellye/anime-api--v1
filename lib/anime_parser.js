...
const BASE_URL = 'https://gogoanimes.mom/';
const BASE_URL2 = 'https://gogoanimes.mom/';
const ajax_url2 = 'https://gogoanimes.mom/';
const ajax_url = 'https://ajax.gogocdn.net/';
const anime_info_url = 'https://gogoanimes.mom/anime/';
const anime_movies_path = 'anime-movies';
...
const recent_release_url = `${ajax_url2}`;
const list_episodes_url = `${ajax_url}ajax/load-list-episode`;
const seasons_url = 'https://gogoanimes.mom/sub-category/';
...
export const scrapeRecentRelease = async ({ list = [], page = 1, type = 1 }) => {
  try {
    ...
    $('div.last_episodes.loaddub > ul > li').each((i, el) => {
      list.push({
        animeId: $(el).find('p.name > a').attr('href').split('/')[4].split('-episode-')[0],
        episodeId: $(el).find('p.name > a').attr('href').split('/')[4],
        name: $(el).find('p.name > a').attr('title'),
